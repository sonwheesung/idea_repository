...
├── cardColor   (nullable — v4, 2026-09-14. 카드 포인트 바 색 키, null = 테마 기본 동작)
├── createdAt
└── updatedAt
```
...
- **세션 협업 규칙: 정본 `C:\project\common\SESSION_PROTOCOL.md`**(형제 앱과 공유하는 정본. 2026-09-14 사용자 승인으로 추가. 값을 여기 베껴 적지 않는다).
...
| W | **프로젝트별 카드 색상**(2026-09-14 사용자 지시 "프로젝트 별 카드 색상 추가 및 설정할 수 있게, 기본값은 현재 색상") — 카드 왼쪽 포인트 바(colorPoint 테마가 쓰던 자리)를 프로젝트 단위 색으로. 색 8종 + 기본, 기본 null = 기존 동작 그대로(colorPoint는 인덱스 순환, 그 외 테마는 바 없음). DB v4 `card_color`(색 키 저장, CHECK 없음 — 색 추가마다 마이그레이션을 안 만들기 위해 코드 검증), hex 정본은 `theme/palettes.ts` `CARD_COLOR_VALUES`, 폼은 기본 정보 Select + OptionSheet 색 점 확장(§14 M 준수). 필터·검색·상세 표시 없음(approach와 같은 취급). 새 문구는 서브 에이전트로 ChatGPT 검수(같은 날 사용자 지시) | 상세 PROJECT_SYSTEM §3.1·§8 · DATABASE §2.3. ⚠ DB 마이그레이션이 실리므로 배포 기본은 AAB(OTA_SYSTEM §1 — 스키마 인상 OTA는 비가역) |
...
- **2026-09-18 vc14 · 1.0.13 빌드(결제 권한 활성화) + RevenueCat 프로젝트 생성 + 릴리스 E2E**(사용자 지시 "크롬으로 진행할 수 있는 거 다"): Play 일회성 상품 페이지가 "결제 권한을 APK에 추가하세요"로 막힌 것 실측 → **vc14 빌드**(`react-native-purchases`로 `com.android.vending.BILLING` 병합, `-CleanNative`·R8, AAB 66.4MB, SHA256 `864958fc…`, D:\builds). **릴리스 R8 E2E 통과**(bundletool universal → AVD: 무크래시·웰컴/홈/설정 렌더·AdMob 배너+App Open 실게재로 정지 해제 재확인·설정 Remove Ads 행 미노출로 `purchasesAvailable()` 게이트 정상·About 1.0.13·FATAL 0). **RevenueCat 브라우저 생성**: 프로젝트 "Idea Repository"(`c31935bf`) + Play 앱 config(`app1a6df7a99e`, package 일치) + Android 공개 키 발급(세션 스크래치패드 기록, 리포 커밋 금지). 진행: ~~vc14 트랙 업로드~~ → ✅ **2026-09-18 내부 테스트 게시**(사용자 AAB 드래그 → 세션이 출시명·노트 입력·릴리스 제출, 게시 16:11) → **Play 일회성 상품 메뉴 열림 실확인**(BUILD §5). → ✅ **Play 상품 `remove_ads` ₩3,300 생성·활성**(2026-09-18 세션이 브라우저로: 제품 ID `remove_ads`·구매 옵션 `remove-ads`·유형 구입·이름/설명 en+ko·173개국). ⏳ 남은 것(사용자·콘솔): RC 서비스계정 JSON 업로드(자격증명, 세션이 안 함)·상품 import·entitlement attach·offering → 키 인라인(`EXPO_PUBLIC_RC_ANDROID_KEY`, `.env` 게이트 때문에 파일 아닌 빌드 명령에)으로 vc15 기능 빌드 → 샌드박스 구매/복원 E2E → 법무(RevenueCat 미국·구매 내역 반영).
- **2026-09-18 RevenueCat 배선 진행·주간 세션 소진으로 일시 중단**(사용자 지시 "다음주에 남은 작업"). 브라우저로 여기까지 완료: RC 프로젝트 "Idea Repository"(`c31935bf`) + Play Store 앱 config(`app1a6df7a99e`, package 일치) + Android 공개 키 발급 · entitlement **`remove_ads`**(Display Name Remove Ads) 생성 · Play 상품 **`remove_ads`**(구매 옵션 `remove-ads` · 유형 구입 · ₩3,300 → 173개국 · **활성**) 생성(이름/설명 en·ko) · **서비스 계정 `revenuecat-play@optimal-shard-426006-n3.iam.gserviceaccount.com` 계정 권한 3개 부여**(앱 정보 보기 및 보고서 일괄 다운로드(읽기 전용) · 재무 데이터·주문·취소 설문 응답 보기 · 주문 및 구독 관리 — 세션이 직접 체크·저장·"예" 확정). Cloud "Play Android Developer API"는 프로젝트 `optimal-shard-426006-n3`에 **이미 사용 설정** 실확인. ⏳ **막힌 지점**: RC 자격증명 검증이 아직 빨강 — 방금 부여한 권한의 **구글 전파 대기**(보통 5~15분·최대 수시간, RC 공식). ⏭ **다음 세션 재개 순서**: ① RC 앱 config(`app.revenuecat.com/projects/c31935bf/apps/app1a6df7a99e`)에서 "Check credentials" **초록 확인**(계정 권한 3개가 앱 정보 보기 포함 다 켜졌는지도 확인 — 안 되면 더 대기) → ② Product catalog에서 상품 `remove_ads` **import** → entitlement `remove_ads`에 **attach** → offering **`default`** 만들고 패키지 추가 → ③ RC Android 공개 키(RC 콘솔 Public API Key 복사 버튼으로 정확히)를 빌드 명령에 **인라인**(`EXPO_PUBLIC_RC_ANDROID_KEY=goog_… NODE_ENV=production powershell -File tools/build-aab.ps1 -VersionCode 15 -VersionName 1.0.14 -CleanNative` — `.env` 게이트 때문에 파일 아님) → vc15 빌드 → ④ 릴리스 R8 E2E + 실기기 샌드박스 구매/복원(**테스트 ID로만**) → ⑤ 법무(RevenueCat 미국이 구매·영수증 받음 → 처리방침·Play 데이터 보안에 "구매 내역" 반영, 게시는 사용자 확인 후) · 약관 §3 확인. ⚠ 서비스 계정 권한은 **켠 채 유지**(끄면 RC 재실패). 코드·문서는 이미 커밋·푸시됨(HEAD). MONETIZATION §4.1. BUILD §5 vc14 · MONETIZATION §4.1.
...
- **2026-09-14 공지 영어 본문 반영**(공통서버 세션 경유 사용자 지시 "idea_repository 도 공지사항 영어 추가할 수 있게"): 서버가 공지에 `titleEn`·`bodyEn`을 붙였고(선택 값, 스위치를 켠 앱만 수신) `idearepository` 스위치가 켜졌다. 앱은 SDK `2026-09-14` 재복사(순수 추가분: `localizeAnnouncement()` + `AnnouncementItem` 선택 필드 2개, 기존 호출부 무변경) + `app/notice.tsx`가 제목·본문을 직접 읽는 대신 `localizeAnnouncement(item, i18n.language)`로 고른다(렌더 지점은 그 한 곳뿐. 설정 배지는 id 수만 센다). 고르는 규칙은 SDK 내장(한국어 기기는 항상 한국어, 그 외는 영어 제목·본문이 둘 다 있을 때만 영어). 읽음 키는 계속 공지 id. 구 설치본은 새 필드를 무시하고 한국어 유지(무오류). ~~⏳ 배포는 사용자 지시 대기~~ → **같은 날 첫 OTA 게시**(사용자 지시 "ota 배포로 하자" — 18:24 KST, 채널 production · runtimeVersion 1.0.0 · 그룹 `12347226-9aa3`. `update:list`·`channel:list` 실조회 + **기기 실측**: vc12 실물 AAB → bundletool universal APK → 프로젝트 AVD 콜드 2회, 1회차 다운로드 완료·2회차 적용 확인·JS 오류 0. 함정 기록 포함 BUILD §5 ota 행). 같은 날 **리포 공개 유지 사용자 결정**("그냥 공개로 할래, 비공개는 유료"): 9/9부터 대기하던 비공개 전환을 하지 않기로 확정, 미푸시 커밋 push 재개. 같은 날 §11에 **세션 협업 규칙 포인터** 한 줄 추가(사용자 승인 "그 한 줄 추가해". 9/9부터 대기하던 마지막 건 해소).
- **2026-09-14~15 프로젝트별 카드 색상 — vc13 · 1.0.12 (§14 W)**: 사용자 지시 "프로젝트 별 카드 색상 추가·설정, 기본값은 현재 색상". 문서 선행(§14 W·PROJECT_SYSTEM §3.1·§8·DATABASE §2.3) → DB v4 `card_color`(색 8종·기본 null=기존 동작·CHECK 없이 코드 검증) · hex 정본 `theme/palettes.ts` `CARD_COLOR_VALUES` · 폼 Select + OptionSheet 색 점 · 카드 왼쪽 포인트 바 반영 · 백업 가져오기 호환 · i18n en·ko. 문구 ChatGPT 검수(카드 색상 라벨 현행 유지). 정적 검증 전부 통과 + **에뮬 실측**(디버그·R8 릴리스 둘 다: v3→v4 마이그레이션 무크래시·색 선택·카드 바). DB 마이그레이션이라 OTA 아닌 AAB(OTA_SYSTEM §1). **vc13 빌드**(`-CleanNative` 없이·R8 유지·매니페스트 8항목 기대값·SHA1 일치, D:\builds 보관) → **내부 테스트 제공**(사용자 콘솔 업로드, 테스터 "사장님 김증 전용", 화면 확인 — 스토어 표시가 안 뜬 원인은 테스터 옵트인 미수락이었고 수락 후 정상) → ~~**프로덕션 100% 출시 검토 중**~~ → ✅ **프로덕션 게시(2026-09-15)**(사용자 콘솔 승격·제출, 버전 13·국가 177·전체 출시·출시 노트 en/ko. 2026-09-18 콘솔 실측 "최신 프로덕션 버전 13 (1.0.12)·9/15·출시율 100%" — vc13이 라이브). ⚠ 프로덕션 출시는 사람이 직접(서비스 계정 프로덕션 권한 불부여 규칙, PLAY_RELEASE_AUTOMATION §5). BUILD §5 vc13 행.

...
- **2026-08-21 로컬 백업 완료**(§3 #23 · §14 P — 무료): 문서 선행([`docs/BACKUP_SYSTEM.md`](./docs/BACKUP_SYSTEM.md)) → expo-file-system/sharing/document-picker → `features/backup` → `/backup` 화면(설정 → 백업) → 약관 5차·처리방침 4차 정본 개정(2026-08-28 시행, **재게시는 사용자 확인 후**) → 에뮬 실측(내보내기 공유 시트·병합·교체·거부). **다음 AAB vc6에 포함**(네이티브 모듈 추가라 AAB 재빌드 필수).
- 다음 단계: vc5 검토 결과 대기 → 백업(vc6) → (AdMob 해제 후) Phase 7(Remove Ads: Play 상품 등록·RC 익명·구매/복원, 데이터 보안에 구매 내역 추가) → 프로덕션 신청(14일 후). 순서는 [`docs/PLAN.md`](./docs/PLAN.md).

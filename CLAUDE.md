...
| O | **발상 단어 관리 화면**(2026-08-19 사용자 지시 "출처 선택 말고 조합 우측 상단 단어 관리 아이콘 → 내장 단어 포함 전부 추가·수정·삭제") — 단어 출처 Select 제거 · DB v3 `ideation_words`(언어별 시드) · `/idea-lab/words`(그룹별 목록·CRUD·내 태그/카테고리 가져오기·기본 단어 복원) | 내장 단어 = 일반 행(기본 카테고리와 같은 원칙). 시드 데이터는 `db/ideation-pool.ts`(의존 방향). 상세 IDEATION_SYSTEM §3.5·§4, DATABASE §2.2 |
...
- **2026-08-19**: 발상 단어 관리 화면(§14 O — DB v3 · `/idea-lab/words` · 출처 Select 제거) — 다음 AAB vc3.

...
| vc5 · 1.0.4 | 2026-08-21 | 단어 관리 헤더 ＋ 아이콘·하단 액션 제거 · 한국어 조사 자동 처리(es-hangul) — CLI 경로 2회차(권한 켬 API 204 → eas submit draft → 출시명·노트 → 검토 전송 → 권한 회수, 리로드 3개 확인·API 403). ⚠ 회수 직후 API 프로브는 수십 초 204 유지(전파 지연) — 콘솔 리로드로 먼저 확인 |

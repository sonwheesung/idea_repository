...
| **vc14 · 1.0.13** | 2026-09-18 빌드·릴리스 E2E 통과 · **2026-09-18 내부 테스트 게시**(사용자 AAB 드래그 업로드 → 세션이 릴리스 제출, 출시명 `14 (1.0.13)`·노트 en/ko, 게시 16:11) → **Play 일회성 상품 메뉴 열림 실확인** | **Remove Ads 결제 인프라 — 결제 권한 활성화 빌드**(CLAUDE §14 결제·MONETIZATION §4.1): `react-native-purchases ^10.10.0` 네이티브 모듈 추가로 매니페스트에 **`com.android.vending.BILLING` 병합**(Play 일회성 상품 메뉴를 여는 열쇠 — 그 전엔 "결제 권한을 APK에 추가하세요"로 막힘). `-CleanNative`(네이티브 추가)·R8 유지. AAB 66.4MB(+2.6MB vs vc13)·mapping 97.4MB·매니페스트 8항목 기대값(versionName 1.0.13·AD_ID·expo-channel-name·EXPO_RUNTIME_VERSION·u.expo.dev True · READ_EXTERNAL_STORAGE·USE_BIOMETRIC False)·**BILLING 권한 병합 실확인**·runtimeVersion 1.0.0 유지. SHA256 `864958fc…`, D:\builds 보관. **릴리스 R8 E2E**(vc14 AAB → bundletool universal APK 90MB → 프로젝트 AVD idea_repository 5572): 첫 실행 무크래시·웰컴 시트·홈·설정 렌더 · **AdMob 배너+App Open 실게재**(정지 해제 실증, 에뮬 Test Ad) · **설정에 Remove Ads/복원 행 미노출 확인**(RC 키 없어 `purchasesAvailable()=false` 게이트 정상) · logcat FATAL 0 · About 1.0.13. ⚠ 이 빌드엔 RC 키가 없어 결제 UI 숨김 — 목적은 BILLING을 트랙에 올려 Play 상품 메뉴를 여는 것. 업로드 → Play 상품 `remove_ads` 등록 → RC offering·키 넣은 다음 빌드(vc15)로 실구매 켜짐. ⚠ 빌드 함정: 스크립트를 PowerShell `*>`로 감싸 돌리면 gradlew의 stderr(NODE_ENV 경고)가 `$ErrorActionPreference=Stop`에서 NativeCommandError로 빌드를 죽인다 — bash에서 `powershell.exe -File … > log 2>&1` + `NODE_ENV=production`으로 해결(정본 호출은 `powershell -File`) |
| **vc13 · 1.0.12** | 2026-09-14 빌드·릴리스 E2E 통과 · **2026-09-15 내부 테스트 제공됨**(사용자 콘솔 업로드, 출시명 `13 (1.0.12)`·테스터 그룹 "사장님 김증 전용" 2명, 게시 9/15 10:45) → **프로덕션 100% 출시 검토 중**(사용자 콘솔 승격·제출 2026-09-15: 버전 13·국가 177·지원 기기 19,250·전체 출시. 출시 노트 en/ko 세션 입력. 라이브는 vc12까지, 심사 통과 시 대체) | **프로젝트별 카드 색상**(CLAUDE §14 W — DB v4 `card_color`). DB 마이그레이션이라 OTA 불가·AAB 필수(OTA_SYSTEM §1). `-CleanNative` 없이 빌드(네이티브 변경 0), R8 유지. AAB 63.8MB·mapping 73.5MB·매니페스트 8항목 전부 기대값(versionName 1.0.12·AD_ID·expo-channel-name·EXPO_RUNTIME_VERSION·u.expo.dev True · READ_EXTERNAL_STORAGE·USE_BIOMETRIC False)·runtimeVersion 1.0.0 유지. SHA1 `26:A5:…:A8:0E` 업로드 키 일치. **릴리스 R8 E2E**(vc13 AAB → bundletool universal APK → 프로젝트 AVD): v3→v4 마이그레이션 무크래시 → 카드 색상 시트(색 점 9개, R8에서도 렌더) → Teal 선택·저장 → 홈 카드 teal 바 · JS 오류 0. ⚠ 빌드가 2회 메모리 부족으로 죽어(에뮬+Metro 동시) 둘 내리고 재빌드. 내부 테스트 트랙 = 회원님(사장님) 테스터 그룹으로 화면 확인용 |
| **ota · 2026-09-14** | 2026-09-14 18:24 KST 게시(사용자 지시 "ota 배포로 하자") | ⭐ **첫 OTA 게시** — 공지 영어 본문(`app/notice.tsx` localizeAnnouncement + SDK 2026-09-14, 커밋 `a1815c7`). 채널 `production`(브랜치 자동 생성) · runtimeVersion `1.0.0` · 그룹 `12347226-9aa3` · Android 업데이트 `01a09f3b-5767`. 발행 전 가드 전부 초록, vc12 이후 네이티브 변경 0(`git diff --name-only`로 확인), 버전 범프 커밋 HEAD에 없음. **게시 확인은 `update:list`·`channel:list` 실조회**(exit 0을 전달로 치지 않음). **기기 실측**: vc12 실물 AAB(D:\builds)를 bundletool universal APK로 변환해(업로드 키 서명, 임베드가 OTA보다 오래된 상태 보존) 프로젝트 AVD에 설치 → 콜드 1회차 logcat에 CheckCompleteAvailable(우리 업데이트 ID)·44에셋 DownloadComplete·isUpdatePending=true → 콜드 2회차에 pending 소진·서버 체크 CheckCompleteUnavailable(기기가 보고한 실행 중 업데이트 = 우리 OTA라는 프로토콜 증거)·JS 오류 0·웰컴 시트 정상 렌더. ⚠ 교훈: OTA 수신 검증에 지금 재빌드한 APK를 쓰면 안 된다. 임베드 commitTime이 OTA보다 새것이 되어 기기가 OTA를 무시한다(`common/OTA_RULES.md` §3.1의 역방향). 업로드했던 AAB 실물을 변환해 쓴다(D:\builds 영구 보관의 실효가 이것) |
| **vc12 · 1.0.11** | 2026-09-09 빌드·업로드·검토 전송(업로드·제출은 사용자) · **당일 게시 9/9 11:44** | **R8 난독화·코드 축소 켬**(Play "앱 최적화 기준점 미만 · 난독화 1%" 경고 대응 — 원인·절차 정본 `common/R8_OBFUSCATION.md`, 프로젝트 반영 §2.5): app.json expo-build-properties(minify·shrinkResources·extraProguardRules) + 스크립트 optimize.txt 패치·게이트·mapping.txt 확인. `-CleanNative` 빌드 63.8MB(vc11 대비 -2.1MB)·mapping 73.5MB·매니페스트 8항목 전부 기대값·runtimeVersion 1.0.0 유지. **릴리스 APK E2E 통과**(오프라인 첫 실행·sqlite·SecureStore 세션·**EASSharedPreferences(expo-updates 생존)**·admob+UMP·라이선스·Idea Lab — 파손 0). 업로드는 서비스 계정 권한 게이트로 **사용자 콘솔 드래그**(세션은 출시명 `12 (1.0.11)`·노트 en/ko 작성 — 최적화만). 검토 당일 통과·게시(9/9 11:44) → **모니터링 및 개선 "주의 필요" 경고 소멸 실측** → latest `1.0.11` PATCH(bootstrap 실측) |

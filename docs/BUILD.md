...
| vc6 · 1.0.5 | 2026-08-21 | **로컬 백업(내보내기·가져오기)** + 약관 5차/처리방침 4차 — CLI 경로 3회차(권한 켬 API 204 → eas submit draft → 출시명·노트 → 검토 전송 → 권한 회수 리로드 3개 확인). 네이티브 모듈 3종 추가(expo-file-system·sharing·document-picker). ⚠ 콘솔 체크박스·버튼은 ref 클릭이 Angular 상태를 안 바꾼다 — 좌표 클릭으로 |

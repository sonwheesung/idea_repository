...
| 영역 | 상태 |
|---|---|
| 프로젝트 생성(이름만) · 상세 · 수정 · 삭제 | 🔨 생성 ✅ · 삭제(카드 길게 누르기) ✅ 2026-08-17 · 상세/수정 Phase 2 |
| 기본 정보(요약·설명·카테고리·태그) | 🔨 생성 폼 "Add details"에 요약·설명·카테고리 ✅ · 태그 ❌ |
| 아이디어 정보(문제점·목표·핵심 아이디어·타겟) | ❌ |
| 진행 관리(진행률·상태·우선순위·시작일·마감일) | 🔨 생성 폼에 상태·우선순위 Select ✅ · 진행률·일정은 편집(Phase 2) |
| 카테고리 관리(시드 7종 + CRUD + 삭제 시 처리) | 🔨 시드 ✅ · 관리 화면 ❌ |
| 태그(자유 생성 · 자동완성 · 자동 정리) | ❌ |
| 아이디어 노트 CRUD | ❌ |
| 관련 자료 CRUD + 외부 브라우저 | ❌ |
| 메인(카드 목록 · 상태 칩 · 검색 · 필터/정렬 시트) | 🔨 카드 목록(Recently Updated) ✅ 2026-08-17 · 칩·검색·필터/정렬 Phase 3 |
| 검색 9필드 | ❌ |
| 필터 3축 · 정렬 6종 | ❌ |
...
- **폼 형태 = Label + input**(2026-08-17 사용자 지시): `TextField`(라벨 위, 입력 아래) · 선택형은 `Select`(라벨 + 현재 값 → 옵션 모달).
  "Add details" 펼침 안에는 요약 · 설명 · 카테고리 · 상태 · 우선순위 — 생성은 빠르게, 나머지(문제점·목표·핵심 아이디어·타겟·진행률·일정·태그)는 상세/편집에서.
...
- 구현: 이동이면 `UPDATE projects SET category_id=? WHERE category_id=?` 후 DELETE — 한 트랜잭션.
  없음이면 FK `ON DELETE SET NULL`이 처리(명시 UPDATE도 무방).
...
- 입력 UX: 칩 입력. 타이핑 중 기존 태그 자동완성(prefix LIKE). 공백·쉼표·엔터로 확정. 태그 안 공백은 허용하지 않는다
  (`Solo Developer` → `SoloDeveloper`로 안내하거나 거부 — 구현 시 안내 문구로 확정).
...
- **모든 항목은 선택** — 비어 있는 항목은 **섹션 제목 + 흐린 "Add …" 플레이스홀더**로 표시한다(숨기지 않는다 —
  "여기에 이런 걸 채울 수 있다"가 발전을 유도한다. 2026-08-17 위임 판단. 숨김 옵션은 P1).
- 우상단 [Edit] → 편집 화면(전 필드 폼). [⋮] → 삭제(확인: "노트 N개·자료 M개도 함께 삭제").

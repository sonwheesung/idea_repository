...
| 영역 | 상태 |
|---|---|
| 프로젝트 생성(이름만) · 상세 · 수정 · 삭제 | ❌ |
| 기본 정보(요약·설명·카테고리·태그) | ❌ |
| 아이디어 정보(문제점·목표·핵심 아이디어·타겟) | ❌ |
| 진행 관리(진행률·상태·우선순위·시작일·마감일) | ❌ |
| 카테고리 관리(시드 7종 + CRUD + 삭제 시 처리) | ❌ |
| 태그(자유 생성 · 자동완성 · 자동 정리) | ❌ |
| 아이디어 노트 CRUD | ❌ |
| 관련 자료 CRUD + 외부 브라우저 | ❌ |
| 메인(카드 목록 · 상태 칩 · 검색 · 필터/정렬 시트) | ❌ |
| 검색 9필드 | ❌ |
| 필터 3축 · 정렬 6종 | ❌ |
...
기본 카테고리의 수정·삭제 허용 여부는 CLAUDE.md §14 미결정 #5(제안: 허용, 일반 행과 동일).

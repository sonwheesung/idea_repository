...
> ⚠ 노트의 "effective 2026-09-08 / 2026-09-08 시행"은 콘솔에 **입력된 그대로**다. 전송 직후 처리방침 시행일이 2026-09-01(당일)로 정정됐지만(LEGAL_SYSTEM §7 #14), 사용자가 "다시 올리자"고 한 시점(9/1 오후)엔 이미 검토 통과·게시돼 취소할 변경사항이 없었다. 게시된 릴리스의 노트는 콘솔에서 수정 불가 — 정정하려면 노트만 다른 vc11이 필요(사용자 결정 대기).

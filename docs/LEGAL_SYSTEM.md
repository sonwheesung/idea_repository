...
| **처리방침 EN rev.6 · KO 6차(2026-09-01 → 2026-09-02 확장)** — Expo EAS Update(OTA) 수탁·국외 이전·수집 항목 + **웜 스타트 하트비트 전송 시점**("앱 실행 시 1회" → "실행 시 + 앱으로 돌아올 때", SDK 2026-09-02) | ✅ 정본·page.tsx 개정 · ✅ **게시(2026-09-02 · 당일 시행)** · ✅ 시행 고지 발행 | 사용자 승인("전부 진행") + 세션 확인 게이트 후 게시 — 게시 기록 참조. 근거 [`OTA_SYSTEM.md`](./OTA_SYSTEM.md) §8 · [`ARCHITECTURE.md`](./ARCHITECTURE.md) §5.7. vc11 업로드 **전** 게시 순서 준수. Data Safety 양식 변경 없음(기기 ID 기선언·항목 불변) — §7 #15 해소 |
...
| 15 | ~~**6차(OTA — Expo EAS Update) 게시·시행 대기**(2026-09-01)~~ → **해소(2026-09-02)** | 게시 200 실측(rev.6 · 2026-09-02 시행, 임시 시행일이 게시일과 일치해 재설정 불요) + 시행 고지 발행(id `9ecde8ec…`) + 게시 기록 완료. vc11 업로드 **전** 게시 순서 지킴(같은 날 오후 프로덕션 검토 전송). ~~LinkMemo 처리방침에 Expo 항목 없음~~ → 전달 완료, LinkMemo 3차(9/1)·4차(9/2)로 자체 해소 | 해소 |

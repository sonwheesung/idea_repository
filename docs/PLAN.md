...
| 1 | DB + 프로젝트 CRUD(이름만 생성) + 카테고리 시드/관리 + 태그 (핵심 도메인) | 🔨 2026-08-17 — DB v1·시드 ✅ · 생성 폼(Label+input, Select) ✅ · 카드 목록 ✅ · 삭제(길게 누르기) ✅ · 실기기 Tailscale 확인 ✅(사용자). 남음: 카테고리 관리 화면 · 태그 입력 |
| 2 | 상세·편집 전 필드 + 아이디어 노트 + 관련 자료 | ⬜ |
| 3 | 탐색 — 검색 9필드 · 필터 3축 · 정렬 6종 | ⬜ |

...
| **2026-09-02 저녁** | ✅ **프로덕션 첫 출시 실행 — "검토 중인 변경사항"**(사용자 확인 게이트 통과): rev.6 게시·시행 고지 → 번들 업로드(`play-upload-bundle.sh`) → 부제·설명·첫 출시형 노트·국가 177 → 변경사항 7개 검토 전송 → 권한 회수 | 세션 | ⚠ 첫 릴리스라 rollout % 없음(전체 출시). 다음: 검토 결과 확인(사용자/세션) → 게시 확인 후 latest `1.0.10` PATCH → 첫 `eas update`(사용자 지시) → 알파 트랙 처분(사용자) |

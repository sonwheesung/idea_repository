    ...
    <main style={{ maxWidth: 760, margin: '0 auto', padding: 32 }}>
      ...
      <section style={card}>
        ...
        <p style={muted}>
          ...
          your <b>platform (Android/iOS) and app version</b>, and the <b>random device identifier (UUID)</b> described
          in 3.c. The identifier contains no personal details; for inquiries it only lets the app show you replies to
          your own inquiries. We do not ask for your name or email address. Your IP address is used transiently for
          rate limiting and is not stored with the inquiry. If you never send an inquiry, no message content is
          transmitted.
        </p>
        ...
        <p style={h3}>c. App start check (announcements, maintenance, required updates) and active-user statistics</p>
        <p style={muted}>
          On launch the app contacts our server once to fetch service notices, maintenance status and version
          requirements. The request carries the app code, platform and app version (plus the standard connection data
          of any internet request, such as your IP address, processed transiently by our hosting provider). None of
          your saved content is included.
        </p>
        <p style={muted}>
          ...
          Deleting the app breaks the link (a reinstall generates a new identifier).
        </p>
        <p style={bullet}>
          • Purpose: service operation; service usage statistics (active-user counts). Legal basis (GDPR): legitimate
          interest in operating and measuring the service. Retention: request data not stored; active-day rows kept
          <b> 400 days</b>, then destroyed; the identifier is deleted on request (section 5).
        </p>
        ...
        <table style={table}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={td}>Inquiry and statistics database hosting</td>
              <td style={td}>Inquiry text, category, platform, app version, device UUID; active-day rows (device UUID + date)</td>
      ...
      <section style={card}>
        ...
        <p style={muted}>
          You can request access to, correction or deletion of the inquiry data described in section 3.b and the
          device identifier and active-day rows described in 3.c at any time via {SUPPORT_EMAIL}. Because we hold no
          name or email, please include the approximate date and content of the inquiry so that we can locate it; for
          the device identifier, the easiest way is to send the deletion request from the in-app inquiry screen
          (Settings → Inquiry), because that message carries the identifier and lets us find exactly your records.
          Data stored on your device is under your control: deleting the app deletes it.
        </p>
        <p style={muted}>
      ...
      <section style={card}>
        ...
        <p style={bullet}>
          • <b>문의하기(선택)</b>: 문의 본문·문의 유형, 플랫폼(Android/iOS)·앱 버전, 아래의 무작위 기기 식별자(UUID) — 이용자가
          문의 화면에서 직접 입력·전송할 때만.
        </p>
        <p style={bullet}>
          • <b>기기 등록(자동, 최초 실행 시 1회)</b>: 앱이 생성한 무작위 기기 식별자(UUID), 앱 코드·플랫폼·앱 버전 — 식별자를 저장하고 세션을
          발급. UUID는 이름·이메일 등 어떤 개인정보도 포함하지 않고 광고 식별자와 다른 값이며, 제3자 제공·광고·추적에 쓰지 않습니다. 용도는 본인 문의의
        ...
        <table style={table}>
          ...
          <tbody>
            <tr>
              ...
              <td style={td}>문의·통계 데이터베이스 호스팅(서울 리전)</td>
              <td style={td}>문의 본문·유형, 플랫폼·앱 버전, 기기 식별자, 활성 기록(기기 식별자·날짜)</td>
            </tr>
            <tr>
              <td style={td}>Vercel Inc.</td>
              <td style={td}>애플리케이션 서버 호스팅(문의·기기 등록·부팅 조회 처리)</td>
        ...
        <table style={table}>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={td}>미국 등</td>
              <td style={td}>문의·기기 등록·부팅 조회 시 전송</td>
              <td style={td}>문의 항목 일체·기기 식별자 세션(전송 처리), 일시적 요청 로그</td>
      ...
      <section style={card}>
        ...
        <p style={bullet}>
          • 회사는 이름·이메일을 보유하지 않으므로 문의 기록의 열람·삭제 요청 시 <b>문의를 보낸 대략의 시점과 내용</b>을 함께 알려 주세요. 기기
          식별자·활성 기록의 삭제는 <b>앱 내 &quot;문의하기&quot;</b>로 요청하시면 식별자가 함께 실려 와 본인 기록을 정확히 특정해 삭제할 수 있습니다.
        </p>

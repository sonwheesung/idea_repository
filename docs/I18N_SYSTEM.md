...
- 🔴 **검사 ④(코드 사용 키)가 19일간 헛돌았던 사고**(2026-09-08 수정 — 공통서버 세션이 전 프로젝트 제어문자 스캔으로 발견):
  ...
  - **일반화**(같은 날 후속): 저장소 전수 상시 가드 **`npm run check:chars`**(`scripts/check-chars.mjs`) 신설 — 텍스트·실행 파일 전체에서
    허용(~~TAB·LF·CR 셋~~ → 같은 날 정정 **TAB·LF·"줄 끝 CR"**) 외 제어문자를 FAIL. ⚠ 허용을 9~13 구간 통짜로 하면 VT(0x0b)·FF(0x0c)가
    빠지고(형제 프로젝트 실측), **CR을 통짜로 허용하면 "줄 중간 CR"(문자열이 조용히 틀림)이 빠진다**(공통서버 세션 정정 — Re:Read 실사고.
    같은 바이트인데 위치가 의미를 가른다 — 판정에 위치 포함). CR 통짜 금지도 답이 아님: autocrlf 체크아웃이 온통 오탐 → 가드가 꺼진다.
    자가 검증 4케이스(정상 CRLF 통과·줄 중간 CR·0x08·0x0b) 내장 — **가드는 자기 구멍을 자기 샘플로 못 보므로** 사고 문자만이 아니라 위치 판정까지 샘플에 든다.
    실측: 줄 중간 CR 주입 → FAIL 발화(덤: printf의 `\\e`→ESC 오염까지 검출 — 이스케이프 계층 함정 재실증) · CRLF 파일 통과 · 제거 → OK.

...
settings.language / settings.categories / settings.removeAds / settings.restorePurchase / settings.notice / settings.inquiry
ads.* / purchase.*
```
...
- ⚠ 성격이 다른 고지 문구는 키를 분리해 재사용을 막는다(조각·LinkMemo 규약) — 데이터 손실 안내(`data.notice.*`)를
  다른 화면 안내와 섞어 쓰지 않는다.
- 복수형·보간은 i18next 기능으로(`{{count}}`), 문자열 이어붙이기 금지.
